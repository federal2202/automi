...
### Files changed
- `prisma/schema.prisma` — replaced `dayOfWeek Int` with `/// managed manually: GIN(daysOfWeek)\n  daysOfWeek Int[]`; removed `@@index([periodId, dayOfWeek])`.
- `prisma/migrations/20260514200000_recurring_activity_days_of_week_array/migration.sql` — hand-written transactional migration: ADD COLUMN daysOfWeek integer[], UPDATE to ARRAY[dayOfWeek], DROP COLUMN dayOfWeek, DROP old B-tree index, CREATE GIN index.
- `prisma/migrations/20260514142532_recurring_activity_days_of_week_array/migration.sql` — auto-generated by Prisma during `migrate dev` (drops the GIN index Prisma can't express in DSL and removes the migration-only DEFAULT; expected side-effect).
- `prisma/migrations/20260514200001_restore_gin_index_days_of_week/migration.sql` — re-adds the GIN index dropped by the auto-generated migration above.
- `src/services/calendar-sync.service.ts` — `generateEventsForActivity`: `dow !== activity.dayOfWeek` → `!activity.daysOfWeek.includes(dow)`. `syncPeriodDateRange`: same replacement in the new-range generation loop. `updateEventsForActivity`: `dayOfWeekChanged` boolean replaced with sort+compare set-equality check on `daysOfWeek` arrays.
- `src/controllers/recurring-activities.controller.ts` — `ActivityInput.dayOfWeek` → `daysOfWeek`; `ParsedActivity.dayOfWeek: number` → `daysOfWeek: number[]`; `ALLOWED_KEYS` updated; `parseAndValidateActivity` now validates Array.isArray, length >= 1, every element integer 0-6, no duplicates via Set; `createRecurringActivity` passes `daysOfWeek`; `updateRecurringActivity` data object uses `daysOfWeek`; `getRecurringActivities` orderBy drops `dayOfWeek` (can't sort arrays), uses `[{ startTime: "asc" }]`.
...
## Steps 4-8 — Full Google Calendar lifecycle sync (2026-05-14)

### Model changes
- `User` — added `timezone String @default("UTC")`.
- New `SyncedEvent` model: `id`, `userId` (FK → User, Cascade), `googleEventId`, `sourceType`, `sourceId`, `date` (UTC midnight of the calendar day), `createdAt`. Unique constraint on `(sourceType, sourceId, date)` — the dedupe gate. Index on `[userId]`. Mapped to `synced_events`. Reverse relation `syncedEvents SyncedEvent[]` added to User.
- Migration: `20260514133100_add_timezone_and_synced_events` (`pnpm exec prisma migrate dev --name add_timezone_and_synced_events`), followed by `prisma generate`.
- New packages: `p-limit@3` (CJS-compatible concurrency limiter), `date-fns-tz@3` (timezone-aware date math).

### New endpoint
- `PATCH /me/timezone` — auth-required. Body `{ timezone }` validated against `Intl.supportedValuesOf("timeZone")`. Rejects unknown keys. Updates `User.timezone`. Returns `{ id, email, name, timezone }`.
- New files: `src/controllers/me.controller.ts`, `src/routes/me.ts`. Mounted in `src/index.ts` as `app.use('/me', meRoutes)`.

### New service
- `src/services/calendar-sync.service.ts` — five exported functions:
  - `generateEventsForActivity(userId, activity, period)` — iterates every day from `max(period.startDate, today_in_user_tz)` through `period.endDate`; filters by `activity.dayOfWeek`; converts wall-clock `startTime`/`endTime` to UTC using `date-fns-tz`; inserts Google event + SyncedEvent row. Unique-constraint violations treated as skipped (idempotent). Returns `{ created, skipped, errors }`.
  - `deleteEventsForActivity(userId, activityId)` — deletes each SyncedEvent's Google Calendar event then the DB row. 404/410 from Google treated as already-gone. Returns `{ deleted, errors }`.
  - `updateEventsForActivity(userId, oldActivity, newActivity, period)` — if `dayOfWeek` changed: delete + regenerate. Otherwise: `calendar.events.patch` each existing event with new title/times. Returns `{ updated, deleted, created, errors }`.
  - `deleteEventsForPeriod(userId, periodId)` — fans out `deleteEventsForActivity` across all activities in the period.
  - `syncPeriodDateRange(userId, period, oldStart, oldEnd)` — deletes SyncedEvents now outside the new range; generates events for days added at either boundary. Returns combined `{ created, skipped, deleted, errors }`.
  - All multi-event operations run Google API calls in parallel with `pLimit(10)`.

### New lib helper
- `src/lib/google-calendar.ts` — `buildCalendarClient(user)` factory: constructs an OAuth2Client with stored credentials, wires a `tokens` event listener that persists refreshed access tokens back to the DB, returns `calendar.events` resource.

### Modified controllers
- `src/controllers/recurring-activities.controller.ts`:
  - `create`: calls `generateEventsForActivity` after DB insert; returns `{ ...activity, sync }`. Google failure never rolls back the DB row.
  - `update`: calls `updateEventsForActivity` with old + new activity; adds `sync` to response.
  - `delete`: calls `deleteEventsForActivity` before DB delete; returns 204 on clean success, 200 with `{ id, sync: { deleted, errors } }` if Google had partial failures.
- `src/controllers/periods.controller.ts`:
  - `delete`: calls `deleteEventsForPeriod` before DB delete.
  - `update`: if `startDate`/`endDate` changed, fires `syncPeriodDateRange` asynchronously (best-effort; errors logged server-side only so the HTTP response is not blocked).

### Files changed
- `prisma/schema.prisma` — User.timezone, SyncedEvent model, User.syncedEvents relation.
- `src/lib/google-calendar.ts` — new.
- `src/services/calendar-sync.service.ts` — new.
- `src/controllers/me.controller.ts` — new.
- `src/routes/me.ts` — new.
- `src/index.ts` — mounted `/me` router.
- `src/controllers/recurring-activities.controller.ts` — wired sync on create/update/delete.
- `src/controllers/periods.controller.ts` — wired sync on delete/update.
- Typecheck: `pnpm exec tsc --noEmit --ignoreDeprecations 6.0` clean.